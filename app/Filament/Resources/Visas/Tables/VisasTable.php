...
class VisasTable
{
    ...
    {
        return $table
            // ✅ Role-based visibility + custom sorting logic
            ->modifyQueryUsing(function (Builder $query) {
                ...
                // ✅ visa_cost = 0 উপরে, বাকি updated_at descending
                return $query
                    ->orderByRaw('visa_cost = 0 DESC') // visa_cost = 0 সবসময় উপরে
                    ->latest('updated_at'); // বাকি ডাটা latest updated
            })
            ...
            ->filters([
                ...
                Filter::make('recent')->label('Recent (Last 7 Days)')->query(fn (Builder $query) => $query->where('created_at', '>=', now()->subDays(7))),
            ])
            ->recordActions([
                ViewAction::make(),
                EditAction::make(),
